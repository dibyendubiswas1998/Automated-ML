...
class Predection_Raw_Data_Validation:
    """
        This class shall be used for validate the raw data for predection.
    ...
    """
    def __init__(self):
        self.file_path = "Executions_Logs/Predection_logs/Raw_Data_Validation_Logs.txt"   # this file path help to log the details in particular file
    ...
    def ValidateMissingValuesInWholeColumn(self):
        ...
        try:
            self.file = open(self.file_path, 'a+')
            for self.fl in listdir("Predection_Raw_Files_Validate/Good_Raw_Data/"):
                self.data = pd.read_csv("Predection_Raw_Files_Validate/Good_Raw_Data/" + self.fl)
                self.count = 0

                for self.column in self.data.columns:
                    if (len(self.data[self.column]) - self.data[self.column].count()) == len(self.data[self.column]):
                        self.count += 1
                        shutil.move("Predection_Raw_Files_Validate/Good_Raw_Data/" + self.fl,
                                    "Predection_Raw_Files_Validate/Good_Raw_Data/")
                        self.logger.log(self.file, "Invalid Column Length for the file!! File moved to Bad Raw Folder :: %s" % self.file)
                        break

                if self.count == 0:
                    self.data.to_csv("Predection_Raw_Files_Validate/Good_Raw_Data/" + self.fl, index=None, header=True)
